...
public class ImgHandler {
    ...
    public void uploadImage(Uri imgURI, uploadCallback callback, String uid, String name) {
        ...
        if (convertedImg == null || !checkImageSize(convertedImg)) {
            Log.d("IMG", "BMP -> B64 conversion failed OR conversion size > 1MB");
            return;
        }
        ...
        imgDocRef.set(imageData)
                .addOnSuccessListener(aVoid -> Log.d("IMG", "Upload transaction success"))
                .addOnFailureListener(e -> Log.d("IMG", "Upload transaction failed"));
        ...
        db.collection(IMAGE_COLLECTION).document(documentID).get()
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists() && documentSnapshot.contains(DatabaseConstants.imgDataKey)) {
                        ...
                        Log.d("IMG", "Requested image doesn't exist or has no image data");;
                    }
                })
                .addOnFailureListener(e -> Log.d("IMG", "Failed to get that document ID. Does it exist?"));
    }
